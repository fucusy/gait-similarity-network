...
def siamses_test(data):
    ...
    distance  = tf.sqrt(tf.reduce_sum(tf.pow(tf.sub(left, right),2),1,keep_dims=True))   
    ...
    with tf.Session() as sess:
        ...
        for i in range(epcho):
            while data.have_next():
                ...
                print(loss_val)
                print("Minibatch loss=%0.2f" % loss_val)
